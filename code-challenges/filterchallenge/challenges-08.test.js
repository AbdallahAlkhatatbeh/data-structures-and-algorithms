...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vowels = /[oaieu]/gi;
  let voweContainer = arr.filter(val => {
    let vowe = val.match(vowels);
    if (vowe) {
      return true;
    }
  });
  return voweContainer;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  let names_container = [];
  x.forEach(element=>{
    names_container.push(element.stat.name);
  });
  return names_container;
};
