...
const isNum = (input) => {
  // Solution code here...
  let regex = /[0-9]/g;
  if (regex.test(input) === true) {return true;} else {return false;}
};
...
const isCapitalized = (str) => {
  // Solution code here...
  let x = str.split(/,| |\)/);
  let firstLetters =/^[A-Z]/;
  let capArr = [];
  x.forEach(element => {
    if(firstLetters.test(element)=== true) { capArr.push(element);}
  });
  return capArr;
