...
const reversedString = (str) => {
  // Solution code here...
  let revereser = str.split('');
  return revereser.reduce((reversed,value)=> {
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((childerenCount,value)=>{
    if(value.children){
      value.children.forEach(() => {
        childerenCount++;
      });
    }
    return childerenCount;
  },0);
};
...
const calculateAverage = (arr) => {
  // Solution code here...
  let avg = arr.reduce((accumulator,value) => {
    accumulator['count']++;
